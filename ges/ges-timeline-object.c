...
GESTimelineObject *ges_timeline_object_copy (GESTimelineObject * object,
    gboolean * deep);
...
ges_timeline_object_split (GESTimelineObject * ref_object, gint64 position)
{
  ...
  g_object_get (ref_object, "duration", &duration, "start", &start, "in-point",
      &inpoint, NULL);

  track_objects = ges_timeline_object_get_track_objects (ref_object);
  layer = ges_timeline_object_get_layer (ref_object);

  new_object = ges_timeline_object_copy (ref_object, FALSE);
